...
#include <iomanip>
#include <fstream>
#include <cstdlib>  //for exit(0) command
#include <vector>
using namespace std;
...
void fullcol(int [][COL],char&,int=6); //reject full column inputs
...
void slcsort(int[],int); //sorts # of turns within four rounds for Player 1
void bublsrt(int[],int); //sort # of turns within four round for Player 2
void swap(int&, int&); //within slcsort  and bublsrt function
...
void average(int block[],int size){
    ...
    rslt/=size;
    cout<<"Average of four turns is "<<fixed<<setprecision(2)<<showpoint<<rslt<<endl;    
}
...
void fullcol (int game[][COL],char& slct,int size){
    vector<char>array={'a','A','b','B','c','C','d','D','e','E','f','F'};
    int c=0,i=0;
    for(;i<size;){
    if((slct==array[c]||slct==array[c+=1])&&(game[0][i]>0)||((slct=='g'||slct=='G')&&(game[0][i]>0))){
        cout<<"Location Full"<<endl;
        prntbrd(game,size);
...
void bublsrt(int block[],int size){
    int maxelm,i;
