...
void lnrsrch(int[],int); //finds a round with 7 turns and outputs a statement
...
int main(int argc, char** argv) {
    ...
    int game[ROW][COL],block[SIZE],trns,i=0,j=0;
    char slct;
    ...
    for(int x=0;x<CONTNUE;x++){
        ...
        if(PL1win(game)==CONTNUE){
            cout<<p+o<<" Wins."<<endl;
            cout<<"Amount of turns until victory was "<<trns<<endl;
            block[i++]=trns;
            if(i==3){
                lnrsrch(block,SIZE);
                average(block,SIZE);
...
void lnrsrch(int block[],int size){
    int i=0;
    bool found=false;
    while(i<size && !found){
        if(block[i]==7){
